...
import "../../standards/ERC721AQueryable.sol";

/// @title The final contract for the roads2Dreams drop
contract RoadsToDreamsAudited is ERC721AQueryable, Ownable, TimeLock, WhitelistAble {
    ...
    function _baseURI() internal view override returns (string memory) { //why
        return _baseTokenURI;
    }

    /// @notice Returns the basic stats of the contract
    function stats() public view  returns (uint256[] memory) { //external
    ...
    function setWhitelistMintData(
        bytes32 whitelistMerkleRoot,
        uint256 whitelistStart,
        uint256 whitelistEnd,
        uint16 maxWhitelistNftMintsPerWallet
    ) public onlyOwner {
        _setWhitelistData(
            whitelistMerkleRoot,
            whitelistStart,
            whitelistEnd,
            maxWhitelistNftMintsPerWallet
        );
    }
    ...
    function _userMint(address newOwner, uint16 amount) internal {
        require(
            _totalMinted() - _ownerMinted + amount <=
                _maxSupply - _ownerReservedMintingAmount,
            "M2"
        );
        require(amount > 0, "M3");
    ...
    function recoverSigner(address minter, bytes calldata signature)
        private
        view
        returns (address)
    {
        return
            keccak256(abi.encodePacked(minter, address(this)))
